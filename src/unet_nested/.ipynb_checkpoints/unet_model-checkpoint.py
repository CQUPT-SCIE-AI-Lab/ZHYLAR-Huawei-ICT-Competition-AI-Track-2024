...
from src.unet_nested.unet_parts import UnetConv2d, UnetUp
...
class UNet(nn.Cell):
    ...
    ###请填写下面代码中缺失的部分
    def __init__(self, in_channel, n_class=???, feature_scale=2, use_deconv=True, use_bn=True):
        ...
        ##1、请补充填写下面代码中缺失的部分
        ##-------------********------------
        filters = [int(x/) for x in ]
        #-------------********------------


        # Down Sample
        ##2、请补充填写下面代码中缺失的部分，定义maxpool2d函数
        ##-------------********------------
        self.maxpool = xxx(kernel_size=, stride=, pad_mode="  ")
        ##-------------********------------


        self.conv0 = UnetConv2d(self.in_channel, filters[0], self.use_bn)
        ##3、请补充填写下面代码中缺失的部分，将unet代码中下采样定义为5层
        ##-------------********------------
        ...
        #/4、请补充填写下面代码中缺失的部分，将unet代码中上采样定义为4层
        ##-------------********------------
...
if __name__ == "__main__":
    import numpy as np
    x = mindspore.Tensor(np.random.randint(0, 255, [1,3,1024, 1536]), mindspore.float32)
    
    
    
    net = UNet(3, n_class=2, feature_scale=2, use_deconv=True, use_bn=True)
    result = net(x)
