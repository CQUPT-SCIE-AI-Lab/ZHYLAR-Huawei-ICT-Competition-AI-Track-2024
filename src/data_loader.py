...
class MultiClassDataset:
    ...
    def _read_img_mask(self, img_id):
        ...
        img = cv2.imread(os.path.join(path, "image.png"))[1000:2800, 2200:4000]
        mask = cv2.imread(os.path.join(path, "mask.png"), cv2.IMREAD_GRAYSCALE)[1000:2800, 2200:4000]
        if self.print_size:
            print("img shape: ", img.shape, "mask shape", mask.shape)  # hwc / hw
...
def preprocess_img_mask(img, mask, num_classes, img_size, augment=False, eval_resize=False):
    ...
    if augment:
        ...
        img = img[dh:dh+img_size[1], dw:dw+img_size[0], :]  # hwc
    ...
    mask = (np.arange(num_classes) == mask[..., None]).astype(int)  # [512,512] -> [512,512,n_cls]
    mask = mask.transpose(2, 0, 1).astype(np.float32)  # hwc->chw
    return img, mask
...
def create_multi_class_dataset(data_dir, img_size, repeat, batch_size, num_classes=2, is_train=False, augment=False,
                               ...
                               num_parallel_workers=2, shuffle=True):
    """
    ...
    dataset = dataset.map(operations=compose_map_func, input_columns=mc_dataset.column_names,
                          ...
                          python_multiprocessing=False)
    dataset = dataset.project(mc_dataset.column_names)
