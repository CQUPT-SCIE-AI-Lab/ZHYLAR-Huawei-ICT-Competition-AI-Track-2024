...
def preprocess_coco_dataset(param_dict):
    ...
    #1、填写参数标签
    #------------------**************
    anno_json = param_dict['anno_json']  # annotaion json文件路径
    ...
    for i, cls in enumerate(coco_cls):
        ...
        coco_cls_dict[cls] = i   # eg:{'backgroud':0, 'person':1',...}
        #------------------**************
    ...
    for ind, img_id in enumerate(image_ids):
        ...
        for instance in anno:
            m = annToMask(instance,h,w)  # h*w的array
            c = coco_cls_dict[classs_dict[instance['category_id']]]  # 最里层为此分割物体属于哪类。class_dict把idx转成类名，cls_dict把类名转回idx
            if len(m.shape) < 3:
                mask[:, :] += (mask==0) * (m *c )  # 当前属于背景的mask=0，与得到的物体的矩阵m，对应位置上标记为类别c
            else:
                mask[:, :] += (mask==0) * (((np.sum(m,axis=2))>0)  * c).astype(np.uint8)  # 将3d转成2d,做上面类似的操作
        #------------------**************
